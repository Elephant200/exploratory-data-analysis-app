...
def get_response(
    ...
    system_prompt:
    str = "You are a helpful assistant that always answers questions.",
    ...
    *,
    ...
    Args:
        ...
        model (str): The Gemini model to use. Defaults to gemini-3.0-flash
    ...
    try:
        ...
        return [
            types.Part.from_text(text=f"Sorry, I ran into an error: {str(e)}")
        ]
...
def upload_file_to_gemini(file) -> types.File:
    ...
    return client.files.upload(
        file=file, config=types.UploadFileConfig(mime_type="text/csv"))
